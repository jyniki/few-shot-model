...
def main(config):
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_dataloader, desc='train', leave=False):
            ...
            data[1] = data[1].cuda()
            data[2] = data[2].cuda()
            x_shot_ori, x_query_ori = split_shot_query(
                data[0], n_way, n_shot, n_query, ep_per_batch=config['train_dataset_args']['batch_size'])
            x_shot_one, x_query_one = split_shot_query(
                data[1], n_way, n_shot, n_query, ep_per_batch=config['train_dataset_args']['batch_size'])
            x_shot_two, x_query_two = split_shot_query(
                data[1], n_way, n_shot, n_query, ep_per_batch=config['train_dataset_args']['batch_size'])
            label = make_nk_label(
                n_way, n_query, ep_per_batch=config['train_dataset_args']['batch_size']).cuda()
            # label = torch.cat([label1, label1], dim=0)

            loss_byol, logits = model(x_shot_ori, x_query_ori, x_shot_one, x_query_one, x_shot_two, x_query_two)
        ...
        for name, loader, name_l, name_a in [
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                x_shot, x_query = split_shot_query(
                    data.cuda(), n_way, n_shot, n_query, ep_per_batch=config['train_dataset_args']['batch_size'])
                ...
                aves[name_l].add(loss_meta.item())
                aves[name_a].add(acc)
