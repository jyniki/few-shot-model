...
## fine-tuning encoder model through ssl ##
def main(config):
    ...
    if config.get('load'):
        ...
        if config.get('load_encoder'):
            ...
            model.encoder_q.load_state_dict(encoder.state_dict())
            model.encoder_k.load_state_dict(encoder.state_dict()) #
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_dataloader, desc='train', leave=False):
            data[0] = data[0].cuda()  # query  [320,3,84,84]
            data[1] = data[1].cuda()  # key    [320,3,84,84]
            
            output, label = model(data[0], data[1])  # output:[320, 10001], label: [320]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',default='configs/train_meta_co_mini.yaml')
