...
def main(config):
    # dataset
    dataset = get_fewshot_dataset(config['dataset_path'],config['dataset'], **config['dataset_args'])
    save_path = os.path.join('./save', config['load'])
    ...
    n_batch = 200
    ep_per_batch = 4
    loader = get_meta_loader(dataset, n_batch=n_batch, ways=n_way, shots=n_shot, query_shots=n_query, batch_size=ep_per_batch)

    # model
    if config.get('load') is None:
        model = models.make('meta-baseline', encoder=None)
    else:
        model = models.load(torch.load(os.path.join('./save',config['load'],'epoch-last.pth')))
    ...
    for epoch in range(1, test_epochs + 1):
        for data, _ in tqdm(loader, leave=False):
            x_shot, x_query = split_shot_query(data.cuda(), n_way, n_shot, n_query, ep_per_batch=ep_per_batch)
            with torch.no_grad():
                if not args.sauc:
                    logits = model(x_shot, x_query).view(-1, n_way)
                    label = make_nk_label(n_way, n_query, ep_per_batch=ep_per_batch).cuda()
                    loss = F.cross_entropy(logits, label)
                    acc = utils.compute_acc(logits, label)
                    aves['tvl'].add(loss.item(), len(data))
                    aves['tva'].add(acc, len(data))
                    va_lst.append(acc)
                else:
