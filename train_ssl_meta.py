...
def main(config):
    ...
    if config.get('load'):
        ...
        config['load_encoder'] = './save/meta_{}-{}shot_meta-{}-ssl-{}/epoch-last.pth'.format(config['train_dataset'], config['n_shot'], config['ssl_type'], config['model_args']['encoder'])
        if config['ssl_type'] == 'byol':
